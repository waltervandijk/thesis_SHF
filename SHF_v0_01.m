...
%% INITIALISATION %%
    ...
    StartTime = 1;        % Time of start  [hour]
    EndTime = 5880;          % Time of end  [hour] (endtime-starttime should be n*24-1
        %StartTime = 1 = '01-Mar-2018 00:00:00';
        %EndTime = 24 = '01-Mar-2018 23:00:00';
    TimeStep = 3600;         % [sec/hour]
    CellSizeX = 5;           % [m]
    CellSizeY = 5;           % [m]
    ThickL = 0.10;           % Layer thickness   [m]
    ThickSurfL = 0.05;       % Surface layer thickness   [m]
    NrLayer = 7;             % Number of layers. Veg = 1, Surface = 2, Layer1-5 use 3:7
    ...
    if exist ('Saturation_int', 'var') && size(Saturation_int,1) == (Subset(2)-Subset(1)+1) 
        ...
        texture_interp_int8=texture_interp_int8(Subset(1):Subset(2),Subset(3):Subset(4));
        ...
        load('lgn2018nh') % load lgn2018; contains NHlat, NHlon & NHdata
            NHdata=NHdata(Subset(1):Subset(2),Subset(3):Subset(4));
            NHdata=double(NHdata);
            NHdata(NHdata==65535)=NaN; %or 17 to make it open water
            NHlon=NHlon(Subset(1):Subset(2),Subset(3):Subset(4));
            NHlat=NHlat(Subset(1):Subset(2),Subset(3):Subset(4));
        ...
        ERA5stl3_adj = ERAstl3 - 273.15;        %temp 2m to [deg C]
        ...
        aa = 500;            % Forest Aerodynamic resistance                 [s/m] 
        ...
        for f = 2:NrLayer
            Temp(:,:,f)= interp2(ERA5longrid,ERA5latgrid,ERA5stl3_adj(:,:,StartTime)',NHlon,NHlat);  % [oC] Soil temp from ERA5
        end
        ...
        % storevars
        StoreInd = 1;
        TempStore = zeros(7,EndTime-StartTime+1);
        FlowStore = zeros(7,EndTime-StartTime+1);
...
while Time <= EndTime    
    ...
    % Plant Height
    PlantHeight = 0.5; % needs real values
    ...
    Kr= calc_kr(TEW, REW, Ssd01_int, rootzone_mm_int); % check for error
    ...
    for hour = 1:24
        ...
        %ERA5ssr_int = interp2(ERA5longrid,ERA5latgrid,ERA5ssr_adj(:,:,Time)',NHlon,NHlat);
...
%         Temp(:,:,NrLayer) = interp2(ERA5longrid,ERA5latgrid,ERA5stl3_adj(:,:,StartTime)',NHlon,NHlat);  % [oC]
        ...
        FlowStore(:,StoreInd) = Flow(1,1,:);
        StoreInd = StoreInd +1;
        
        % Calculate slope vapour pressure, saturation and actual vapour.
        %d = calc_d(ERA5t2m(:,:,(Time)));
        %[Es, Ea] = calc_es_ea(ERA5t2m_int, ERA5d2m_int);   
...
%%         % Calculate Potential ET (ET0)
        lET0 = calc_eto_makkink(ERA5net_rad_int,ERA5t2m_int,TimeStep);     % [m s-1]
        ...
        lETact = calc_etact_evap(lET0, Kcb, Ks, Ke);  % [W m-2]
...
%         save(outputloc,'SHF')
        ...
        save('TempStoreFlowerBulb.mat','TempStore','FlowStore')
