...
%% INITIALISATION %%
    ...
    StartDay = 80;          % day of year = StartDay+60
    StartTime = StartDay*24+1;        % Time of start  [hour] (must be n*24+1)
    EndTime = StartTime+359;          % Time of end  [hour] (endtime-starttime should be n*24-1
    ...
    ThickWaterSurfL = 1;   % Thickness water mixing layer [m]
    ...
    if exist ('Saturation_int', 'var') && size(Saturation_int,1) == size(SubRow,1)
        ...
        EddyConducMult = 1000; % K_Water multiplier for water bodies 
        ...
        Temp(:,:,1) = ERA5t2m_int; % [K] Initial vegetation top layer temperature = air temp
        Temp(:,:,2) = ERA5t2m_int; % [K] Initial soil surface layer temperature = air temp
        ERA5stl4_adj(isnan(ERA5stl4_adj))=min(min(ERA5stl4_adj(:,:,StartTime))); % initial sea temp
        for f = 2:NrLayer
            Temp(:,:,f)= interp2(ERA5longrid,ERA5latgrid,ERA5stl4_adj(:,:,StartTime)',NHlon,NHlat);  % [oC] Soil temp from ERA5
        end
