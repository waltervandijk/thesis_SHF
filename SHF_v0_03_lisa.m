...
% total area : 15000*11000 cells
% splited area: 500*11000 cells
% 16 runs per node (because a node has 16 cores)
% 2 nodes needed (with at least 64 GB memory each)
% variable 'input' should be 1,2,3,4,... 30
...
%% INITIALISATION %%
    ...
    %% Add folders to path
    mkdir(['Output',num2str(input)])
    addpath(['Output',num2str(input)],genpath('functions'),genpath('data'))
    disp('path initialised')

    %% Control Constants
    Debugging = 0;           % put on zero for more efficient memory usage
    StartDay = 0;          % day of year = StartDay+60
    StartTime = StartDay*24+1;        % Time of start  [hour] (must be n*24+1)
    EndTime = StartTime+5879;          % Time of end  [hour] (endtime-starttime should be n*24-1
    ...
    ThickWaterSurfL = 0.5;   % Thickness water mixing layer [m]
    ThickSeaSurfL = 1;     % Thickness sea mixing layer [m]
    NrLayer = 10;             % Number of layers. Veg = 1, Surface = 2, Layer1-5 use 3:7

    %SurfaceT = 25;           % Starting surface temperature                  [oC]
    %SoilT= 20;               % Starting soil temperature                     [oC]
    
    SubRow = input*500-499:input*500;
    SubCol = 1:11000;
    
%     SubRow = 1:15000;
%     SubCol = 1:11000;  
        %Subset = [1501,2000,1,11000]; % obsolete, use SubRow & SubCol
        % Some interesting areas %
            ...
            % Subset=[8601,9600,1601,2600];       %bergen aan zee
            %Subset=[9801,10800,2901,3900];     %alkmaardermeer
    ...
    if exist ('Saturation_int', 'var') && size(Saturation_int,1) == size(SubRow,1)
        ...
        % loading data
        disp('loading data.')
        load('LAIstats.mat'); % LAI stats used to make LAI maps from land use
        ...
        ERA5stl4_adj = ERAstl3; % - 273.15;        %temp 2m to [deg C]
        ERA5u_v_comb_2m = ERA5u_v_comb .* 0.748;      % Convert 10m wind to 2m wind  [m s-1]             [m s-1]
        ERA5stl4_adj(isnan(ERA5stl4_adj))=min(min(ERA5stl4_adj(:,:,StartTime))); % initial sea temp
        ...
        KNMI_soiltemp = 284.21;          % soil lower boundary [K] mean soil temp KNMI at 1m
        ...
        % Albedo
        aSoilWet = 0.15; aSoilDry = 0.30;           % source:
        aRoad = 0.1; aUrban = 0.2; aWater = 0.06; 
        ...
        ERA5t2m_init = zeros(size(NHdata));
        for t = StartTime:StartTime+168
            ERA5t2m_init = ERA5t2m_init+ interp2(ERA5longrid,ERA5latgrid,ERA5t2m_adj(:,:,t)',NHlon,NHlat); % interpolate to get the right size
        end
        ERA5t2m_init = ERA5t2m_init / 168; % initial temp is average over 7 days
        
        Temp = zeros([size(NHdata),NrLayer]);                           % Create Temperature 3d matrix                          
        Temp(:,:,1) = ERA5t2m_init;                                                 % [K] Initial vegetation top layer temperature
        Temp(:,:,2) = ERA5t2m_init;                                                 % [K] Initial soil surface layer temperature
        Temp(:,:,3) = (ERA5t2m_init+KNMI_soiltemp)/2;                               % [K] average temp first soil layer
        for f = 4:NrLayer-1
%             Temp(:,:,f)= interp2(ERA5longrid,ERA5latgrid,ERA5stl4_adj(:,:,StartTime)',NHlon,NHlat,"linear",0);  % [oC] Soil temp from ERA5
            Temp(:,:,f) = (Temp(:,:,f-1)+KNMI_soiltemp)/2; % make a gradient of temps through soil
        end
        Temp(:,:,NrLayer) = KNMI_soiltemp;          % [K] lowest layer KNMI mean soil temp
        
        % Initialize flows and heat content.
        Flow = zeros([size(NHdata),NrLayer]);                         % [] Create the flow 3d matrix                             
        HeatCont= zeros([size(NHdata),NrLayer]);                        % [] Create heatcontent matrix
        ...
        save([pwd,'/Output',num2str(input),'/TimeKey'],'TimeKey');
...
while Time <= EndTime    
    ...
    mkdir([pwd,'/Output',num2str(input),'/MatFiles/day',num2str(daynum)])
    ...
    % Make LAI grid
    LAIgrid=constructLAI(LAIdata, NHdata, decadenum);
    % % % % % need to check LAI values for edge effects for roads&stuff
    
    % Plant Height
    PlantHeight = exp(Rh/100)*2-3; % needs real values
    ...
    for hour = 1:24
        ...
        Albedo = calc_albedo(NHdata,Saturation_int,aSoilDry,aSoilWet,aRoad,aUrban,aWater);
        
        % Calculate Surface radiation.
        SurfaceRad = ERA5str_adj_int + ERA5ssr_adj_int .* (1-CoverFraction) .* (1-Albedo);    % [J m-2]
        if Debugging == 0
            clear('CoverFraction')
        end
        
        % Calculate temperatures
        % ERA5t2m as upper boundary condition. This temperature is taken 
            % for the vegetation layer.
        Temp(:,:,1) = ERA5t2m_int;                   % [oC]
        
        %KNMI data as soil boundary condition
        Temp(:,:,NrLayer) = KNMI_soiltemp;
...
%         Temp(:,:,NrLayer) = interp2(ERA5longrid,ERA5latgrid,ERA5stl3_adj(:,:,StartTime)',NHlon,NHlat);  % [oC]
        ...
        Temp(:,:,2) = Temp(:,:,2) + (SurfaceRad ./ (VHC_Soil .* ThickSurfL)); % [oC]
        if Debugging == 0
            clear('SurfaceRad')
        end
        % Readjust heatcontent based on temperature (change in heat
        ...
        lETact = calc_etact(lET0, Kcb, Ks, Ke);  % [J]
...
%         drawnow
        ...
        SHF = int16(SHF);
        outputloc=[pwd,'/Output',num2str(input),'/MatFiles/day',num2str(daynum),'/SHF_hour',num2str(Time),'_',num2str(input),'.mat'];
        save(outputloc,'SHF')
...
% disp('running SHF_output to create a video')
% pause(1)
% SHF_output
